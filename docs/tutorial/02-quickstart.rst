...
Datalineup main input is the topology file defining all the components: Executor, topics,
...
A Datalineup pipeline is a simple function. Here it simply create a new message with a
capitalized word.
...
Now we are ready to launch our job within Datalineup.

Let's start the worker manager. This service will load our topology and expose it to any
Datalineup workers that are ready to run jobs. The worker manager also track state so that
Datalineup workers can come and go without losing any progress.

Run the following command in a terminal:

.. code:: bash

   python -m datalineup_engine.worker_manager.server
...
The worker manager will be waiting for a Datalineup worker to connect to it.

Let's start it with the following command in a second terminal:

.. code:: bash

   python -m datalineup_engine.worker.runner

You should quickly see the worker start and print some lines in the terminal:

.. code::

   2023-09-28 16:33:30,539 [INFO] datalineup.worker.broker.Broker: Initializing worker
   2023-09-28 16:33:30,539 [INFO] datalineup.worker.broker.Broker: Starting worker
   2023-09-28 16:33:30,554 [INFO] datalineup.worker.broker.Broker: Worker sync
...
You have now run your Datalineup job, now we should go through each of the component we just
