...
from datalineup_engine.utils.declarative_config import UncompiledObject
from datalineup_engine.utils.declarative_config import load_uncompiled_objects_from_path
from datalineup_engine.utils.options import fromdict
from datalineup_engine.utils.tester.config.topic_test import TopicTest
from datalineup_engine.utils.tester.topic_test import run_datalineup_topic
from datalineup_engine.utils.tester.topic_test import run_datalineup_topic_test
from datalineup_engine.worker_manager.config.declarative import compile_static_definitions
...
from .inventory_test import run_datalineup_inventory
from .inventory_test import run_datalineup_inventory_test
from .pipeline_test import run_datalineup_pipeline_test


@dataclasses.dataclass
class DatalineupTests:
...
def compile_tests(uncompiled_objects: list) -> DatalineupTests:
    ...
    tests: DatalineupTests = DatalineupTests()

    for uncompiled_pipeline_test in objects_by_kind.pop(
        "DatalineupPipelineTest",
        dict(),
    ).values():
...
def run_tests(*, static_definitions: StaticDefinitions, tests: DatalineupTests) -> None:
    for pipeline_test in tests.pipeline_tests.values():
        print(f"Running {pipeline_test.metadata.name}...")
        run_datalineup_pipeline_test(
...
def show_inventory(
    ...
    for item in run_datalineup_inventory(
        static_definitions=static_definitions,
        inventory_name=name,
        limit=limit,
        after=after,
...
def show_topic(topology: str, name: str, limit: int, skip: int) -> None:
    ...
    for message in run_datalineup_topic(
        static_definitions=static_definitions,
        topic_name=name,
        limit=limit,
        skip=skip,
