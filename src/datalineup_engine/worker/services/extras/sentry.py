...
from datalineup_engine.utils.traceback_data import FrameData
...
def set_event_tags_from_queue(event: "Event", *, queue: QueueItem) -> None:
    ...
    tags["datalineup.job.name"] = queue.name
    for k, v in queue.labels.items():
        tags[f"datalineup.job.labels.{k}"] = v


def set_event_tags_from_xmsg(event: "Event", *, xmsg: ExecutableMessage) -> None:
    tags = event.setdefault("tags", {})
    tags["datalineup.message.id"] = xmsg.id
    tags["datalineup.pipeline.name"] = xmsg.message.info.name
    for k, v in xmsg.message.message.tags.items():
        tags[f"datalineup.message.tags.{k}"] = v
...
class Sentry(Service[BaseServices, "Sentry.Options"]):
    ...
    async def open(self) -> None:
        self.logger = logging.getLogger("datalineup.extras.sentry")
        sentry_sdk.init(
            self.options.dsn,
        ...
        with hub.push_scope() as scope:
            ...
            def _event_processor(event: "Event", hint: "Hint") -> "Event":
                with capture_internal_exceptions():
                    ...
                    extra = t.cast(
                        dict, event.setdefault("extra", {}).setdefault("datalineup", {})
                    )
        ...
        with hub.push_scope() as scope:
            ...
            def _event_processor(event: "Event", hint: "Hint") -> "Event":
                with capture_internal_exceptions():
                    ...
                    event["tags"]["datalineup.channel.name"] = publish.output.channel
    ...
    def _capture_exception(self, exc_info: Exception, hub: Hub) -> None:
        ...
        if isinstance(exc_info, RemoteException):
            event, hint = event_from_remote_exception(
                ...
                mechanism={"type": "datalineup", "handled": False},
            )
        else:
