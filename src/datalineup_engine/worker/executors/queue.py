...
from datalineup_engine.utils import ExceptionGroup
from datalineup_engine.utils.asyncutils import Cancellable
...
from datalineup_engine.worker.error_handling import process_pipeline_exception
from datalineup_engine.worker.resources.manager import ResourceUnavailable
...
class ExecutorQueue:
    ...
    async def run_queue(self) -> None:
        while self.is_running:
            ...
            with contextlib.suppress(BaseException), processable.datalineup_context():
                async with (
    ...
    async def process_results(
        ...
        *,
        ...
        with contextlib.suppress(Exception), xmsg.datalineup_context():
            async with context:
                await scope(
                    ResultsProcessed(
                        xmsg=xmsg,
    ...
    async def delayed_submit(self, processable: ExecutableMessage) -> None:
        ...
        with processable.datalineup_context():
            try:
