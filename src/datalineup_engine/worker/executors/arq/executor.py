...
from datalineup_engine.utils.asyncutils import cached_property
...
class ARQExecutor(Executor):
    ...
    def __init__(self, options: Options, services: Services) -> None:
        ...
        meter = get_meter("datalineup.metrics")
        self.execute_bytes = meter.create_counter(
            name="datalineup.executor.arq.execute",
            unit="By",
            description="""
        ...
        self.results_bytes = meter.create_counter(
            name="datalineup.executor.arq.results",
            unit="By",
            description="""
    ...
    async def process_message(self, message: ExecutableMessage) -> PipelineResults:
        ...
        tasks = TasksGroup(name=f"datalineup.arq.process_message({message.id})")
