...
class StaticDefinitions:
    ...
    def add(self, obj: BaseObject) -> None:
        from datalineup_engine.worker_manager.config.declarative_dynamic_topology import (
            DynamicTopology,
        )
        from datalineup_engine.worker_manager.config.declarative_executor import Executor
        from datalineup_engine.worker_manager.config.declarative_inventory import Inventory
        from datalineup_engine.worker_manager.config.declarative_job import Job
        from datalineup_engine.worker_manager.config.declarative_job_definition import (
            JobDefinition as DeclarativeJobDefinition,
        )
        from datalineup_engine.worker_manager.config.declarative_resource import Resource
        from datalineup_engine.worker_manager.config.declarative_resource import (
            ResourcesProvider,
        )
        from datalineup_engine.worker_manager.config.declarative_topic_item import TopicItem
