...
from datalineup_engine.models import Base
from datalineup_engine.utils import lazy
from datalineup_engine.utils.inspect import import_name
from datalineup_engine.utils.sqlalchemy import AnySyncSession
from datalineup_engine.utils.sqlalchemy import is_sqlite3_connection
...
def engine() -> Engine:
    ...
    database_connection_creator: Optional[str] = (
        current_app.datalineup.config.database_connection_creator
    )
    extra_args = {}
    if database_connection_creator:
        extra_args["creator"] = import_name(database_connection_creator)
    return create_engine(
        current_app.datalineup.config.database_url,
        future=True,
        pool_recycle=current_app.datalineup.config.database_pool_recycle,
        pool_pre_ping=current_app.datalineup.config.database_pool_pre_ping,
        **extra_args,
    )
