...
from datalineup_engine import database
...
from datalineup_engine.worker_manager import server as worker_manager_server
...
@pytest.fixture
def app(config: Config) -> t.Iterator[DatalineupApp]:
    app = worker_manager_server.get_app(
        app_config={
            "TESTING": True,
        },
        datalineup_config=config,
    )
...
def client(
    ...
    app.datalineup._static_definitions = static_definitions
