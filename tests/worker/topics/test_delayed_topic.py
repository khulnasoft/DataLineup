...
from datalineup_engine.worker.topics import DelayedTopic
...
async def test_delayed_topic(
    ...
    with mock.patch("datalineup_engine.worker.topics.delayed.utcnow", new=publish_time):
        for message in messages:
            await topic.publish(message, wait=True)

    with mock.patch("asyncio.sleep", new_callable=AsyncMock) as mock_sleep:
        with mock.patch("datalineup_engine.worker.topics.delayed.utcnow", new=run_time):
