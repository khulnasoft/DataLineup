    ...
    return config.load_object(
        {
            "services_manager": {
                "services": [
                    "datalineup_engine.worker.services.extras.sentry.Sentry",
                ]
            }
        }
    )
...
async def test_sentry(
    ...
    assert (
        event["extra"]["datalineup"]["pipeline_message"]["info"]["name"]
        == "tests.conftest.pipeline"
    )
    assert event["tags"]["datalineup.job.name"] == "fake-queue"
    assert event["tags"]["datalineup.job.labels.owner"] == "team-datalineup"
